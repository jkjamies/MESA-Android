...
class TrapezeMessageManager {
    private val _message = MutableStateFlow(emptyList<TrapezeMessage>())

    /**
     * A flow emitting the current message to display.
     */
    val message: Flow<TrapezeMessage?> = _message.map { it.firstOrNull() }.distinctUntilChanged()

    fun emitMessage(message: TrapezeMessage) {
